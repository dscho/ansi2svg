...
var bl = require("bl");
var ansi2svg = require("..");

function expect(expected, t) {
  return bl(function(err, buf) {
    if (err) throw err;
    t.equal(buf.toString(), expected, "svg equals expected");
  });
}
...
test("should tranform color sequences to class styled spans if passed option style:class", function(t) {
  ...
  s.push(null);
  s
    .pipe(ansi2svg({ style: "class" }))
    .pipe(expect("<span class=\"ansi2svg-color-green\">Hello world</span>", t));
});
...
test("should add prefix to classes", function(t) {
  ...
  s.push(null);
  s
    .pipe(ansi2svg({ style: "class", prefix: "somePrefix-" }))
